...
class Action extends TradingService\Common\Action\HttpAction
{
	...
	protected function getStatusSkippedSearchMap()
	{
		...
		$lastOrder = (int)$statusService->getStatusOrder($last);
		$storedOrder = (int)$statusService->getStatusOrder($stored);
		$result = [];

		if ($storedOrder > $currentOrder)
		{
			$until = $stored;
			$untilOrder = $storedOrder;
		}
		else
		{
			$until = $current;
			$untilOrder = $currentOrder;
		}

		foreach ($statusService->getProcessOrder() as $intermediateStatus => $intermediateOrder)
		{
			if ($intermediateOrder <= $lastOrder || $intermediateOrder > $untilOrder) { continue; }
			if ($intermediateOrder === $untilOrder && $intermediateStatus !== $until) { continue; }
	...
	protected function getStatusInSearchVariants()
	{
		...
		return $this->makeStatusInSearchVariants($status);
	}
	...
	{
		return $this->provider->getOptions()->getStatusIn($status);
	}
