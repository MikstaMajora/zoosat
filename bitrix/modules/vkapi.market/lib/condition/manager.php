...
class Manager
{
    ...
    public function show($conditionValues = [])
    {
        ...
        ?>;
            ...
            window.BXmakerConditionsJs = window.BXmakerConditionsJs || {};
            window.BXmakerConditionsJs['<?php 
     ...
     * @throws \Bitrix\Main\ArgumentException
     */
    public function getPreparedElementFieldsById(array $arElementId, $bOffer = false)
    {
        $arReturn = [];
        ...
        if (!empty($arReturn)) {
            ...
            while ($arSection = $dbrSection->fetch()) {
                ...
                if (isset($arSectionParentList[$arSection['IBLOCK_SECTION_ID']])) {
                    $arSectionIdList = array_merge($arSectionIdList, $arSectionParentList[$arSection['IBLOCK_SECTION_ID']]);
                }
                $arReturn[$arSection['IBLOCK_ELEMENT_ID']]['IBLOCK_SECTION_ID'] = array_merge($arReturn[$arSection['IBLOCK_ELEMENT_ID']]['IBLOCK_SECTION_ID'], $arSectionIdList);
                if (!$bOffer) {
                    $arReturn[$arSection['IBLOCK_ELEMENT_ID']]['PARENT_IBLOCK_SECTION_ID'] = array_merge($arReturn[$arSection['IBLOCK_ELEMENT_ID']]['PARENT_IBLOCK_SECTION_ID'], $arSectionIdList);
                }
            }
            // убираем будликаты
        ...
        if (self::isInstalledCatalogModule() && count($arReturn)) {
            ...
            while ($arProduct = $dbrProductQuantity->fetch()) {
                $arReturn[$arProduct['ID']]['CATALOG_WEIGHT'] = intval($arProduct['WEIGHT']);
                $arReturn[$arProduct['ID']]['CATALOG_QUANTITY'] = intval($arProduct['QUANTITY']);
                $arReturn[$arProduct['ID']]['CATALOG_AVAILABLE'] = $arProduct['AVAILABLE'] == 'N' ? 'N' : 'Y';
            }
            // на складах ---------
            ...
            while ($arPrice = $dbrPrice->Fetch()) {
                // рассчитываем скидки -----
                $arDiscounts = $oCatalogDiscount->GetDiscountByPrice($arPrice["ID"], [], "N");
                $discountPrice = $oCatalogProduct->CountPriceWithDiscount($arPrice["PRICE"], $arPrice["CURRENCY"], $arDiscounts);
                ...
                if (intval($discountPrice) && $discountPrice < $arPrice['PRICE']) {
                    $arReturn[$arPrice['PRODUCT_ID']]['CATALOG_PRICE_' . $arPrice['CATALOG_GROUP_ID']] = $discountPrice;
                    $arReturn[$arPrice['PRODUCT_ID']]['CATALOG_DISCOUNT_PERCENT_' . $arPrice['CATALOG_GROUP_ID']] = round(floatval($arPrice['PRICE'] - $discountPrice) / $arPrice['PRICE'] * 100);
                    $arReturn[$arPrice['PRODUCT_ID']]['CATALOG_DISCOUNT_PRICE_' . $arPrice['CATALOG_GROUP_ID']] = round($arPrice['PRICE'] - $discountPrice);
                }
                $arReturn[$arPrice['PRODUCT_ID']]['CATALOG_GROUP_' . $arPrice['CATALOG_GROUP_ID']] = preg_replace('/(\\.[0]+)$/', '', $arPrice['PRICE']);
            }
            unset($arDiscounts);
        }
            ...
            switch ($child['id']) {
                ...
                case 'IBLOCK_SECTION_ID_' . $iblockId:
                    ...
                    if ($child['values']['condition'] === 'EQUAL') {
                        $inSection[] = $child['values']['value'];
                    } else {
                        $notInSection[] = $child['values']['value'];
                    }
                    break;
                case 'DEFAULT':
                        ...
                        switch ($subChild['id']) {
                            case 'IBLOCK_SECTION_ID_' . $iblockId:
                                ...
                                if ($subChild['values']['condition'] === 'EQUAL') {
                                    $subInSection[] = $subChild['values']['value'];
                                } else {
                                    $subNotInSection[] = $subChild['values']['value'];
                                }
                                break;
                        }
                    }
                    if (count($subInSection)) {
                        $subFilter->whereIn('IBLOCK_SECTION_ID', $subInSection);
                    }
                    if (count($subNotInSection)) {
                        $subFilter->whereNotIn('IBLOCK_SECTION_ID', $subNotInSection);
                    }
                    $filter->where($subFilter);
            }
        }
        if (count($inSection)) {
            $filter->whereIn('IBLOCK_SECTION_ID', $inSection);
        }
        if (count($notInSection)) {
            $filter->whereNotIn('IBLOCK_SECTION_ID', $notInSection);
        }
        return $filter;
    }
}
