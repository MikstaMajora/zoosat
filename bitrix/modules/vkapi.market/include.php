...
class VKapi_Market_Manager_Demo
{
     ...
     * @return string
     */
    public function truncateTextVK($text, $limit, $curIsUtf = \null)
    {
        if (\is_null($curIsUtf)) {
            $curIsUtf = $this->base()->isUtf();
        }
        if ($curIsUtf) {
            $text = \Bitrix\Main\Text\Encoding::convertEncoding($text, 'UTF-8', 'Windows-1251');
        }
        $result = '';
        do {
            $vkText = $this->prepareVkString($text);
            if (\strlen($vkText) <= $limit) {
                $result = $text;
                break;
            }
            $len = \strlen($text);
            $lenCount = 3;
            for ($i = 0; $i < $len; $i++) {
                $lenCount += \strlen($this->prepareVkString($text[$i]));
                if ($lenCount > $limit) {
                    break;
                }
                $result .= $text[$i];
        ...
        if ($curIsUtf) {
            $result = \Bitrix\Main\Text\Encoding::convertEncoding($result, 'Windows-1251', 'UTF-8');
        }
        return $result;
    }
    /**
     ...
     * проверка демо
     */
    private final function __checkDemo()
    {
        $module = new \CModule();
 ...
 */
    ...
    public function checkExportConditionsForElementId($exportId, $elementId)
    {
        ...
        // берем выгрузку
        $arExport = $this->getExportData($exportId);
        ...
        if ($this->isInstalledCatalogModule()) {
            ...
            if (\is_array($offersExist) && isset($offersExist[$productId]) && $offersExist[$productId] === \true) {
                ...
                $bOffer = \true;
                $offerId = 0;
                $arElementAll = $oCondition->getPreparedElementFieldsById([$productId], \false);
        ...
        $arOffer = \null;
        $arOfferSource = \null;
        $arOfferAll = $oCondition->getPreparedElementFieldsById([$offerId], $bOffer);
