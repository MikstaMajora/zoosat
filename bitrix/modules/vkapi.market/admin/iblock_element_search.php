<?
    ...
    if ($IBLOCK_ID > 0) {
        ...
        $arIBlock = array(
            "ID" => 0,
            "ELEMENTS_NAME" => $oMessage->get("ELEMENTS"),
        );
    }
    ...
    $arHeader[] = array(
        "id" => "TIMESTAMP_X",
        "content" => $oMessage->get("FIELD_TIMESTAMP_X"),
        "sort" => "timestamp_x",
        "default" => true
    );
    $arHeader[] = array(
        "id" => "USER_NAME",
        "content" => $oMessage->get("FIELD_MODIFIED_BY"),
        "sort" => "modified_by",
        "default" => true
    );
    $arHeader[] = array(
        "id" => "ACTIVE",
        "content" => $oMessage->get("FIELD_ACTIVE"),
        "sort" => "active",
        "align" => "center",
        "default" => true
    );
    $arHeader[] = array("id" => "NAME", "content" => $oMessage->get("FIELD_NAME"), "sort" => "name", "default" => true);
    
    $arHeader[] = array(
        "id" => "ACTIVE_FROM",
        "content" => $oMessage->get("FIELD_ACTIVE_FROM"),
        "sort" => "date_active_from"
    );
    $arHeader[] = array(
        "id" => "ACTIVE_TO",
        "content" => $oMessage->get("FIELD_ACTIVE_TO"),
        "sort" => "date_active_to"
    );
    $arHeader[] = array(
        "id" => "SORT",
        "content" => $oMessage->get("FIELD_SORT"),
        "sort" => "sort",
        "align" => "right"
    );
    ...
    $arHeader[] = array(
        ...
        "content" => $oMessage->get("FIELD_CREATED_USER_NAME"),
        "sort" => "created_by"
    );
    ...
    if (CModule::IncludeModule("workflow")) {
        $arHeader[] = array(
            "id" => "WF_STATUS_ID",
            "content" => $oMessage->get("FIELD_STATUS"),
            "sort" => "status",
            "default" => true
        );
    ...
    $arHeader[] = array(
        "id" => "SHOW_COUNTER",
        "content" => $oMessage->get("FIELD_SHOW_COUNTER"),
        "sort" => "show_counter",
        "align" => "right"
    );
    $arHeader[] = array(
        "id" => "SHOW_COUNTER_START",
        "content" => $oMessage->get("FIELD_SHOW_COUNTER_START"),
        "sort" => "show_counter_start",
        "align" => "right"
    );
    $arHeader[] = array(
        "id" => "PREVIEW_PICTURE",
        "content" => $oMessage->get("FIELD_PREVIEW_PICTURE"),
        "align" => "right"
    );
    $arHeader[] = array("id" => "PREVIEW_TEXT", "content" => $oMessage->get("FIELD_PREVIEW_TEXT"));
    $arHeader[] = array(
        "id" => "DETAIL_PICTURE",
        "content" => $oMessage->get("FIELD_DETAIL_PICTURE"),
        "align" => "center"
    );
    ...
    $rsData = CIBlockElement::GetList($arOrder, $arFilter, false,
        array("nPageSize" => CAdminResult::GetNavSize($sTableID)), $arSelectedFields);
    ...
    function GetElementName($ID)
    {
        ...
        if (!array_key_exists($ID, $cache) && $ID > 0) {
            $rsElement = CIBlockElement::GetList(array(), array("ID" => $ID, "SHOW_HISTORY" => "Y"), false, false,
    ...
    while ($arItem = $rsData->GetNext()) {
        ...
        $row->AddViewField("NAME",
            $arItem["NAME"] . '<input type="hidden" name="n' . $arItem["ID"] . '" id="index_' . $arItem["ID"] . '" value="' . $arItem["ID"] . '"><div style="display:none" id="name_' . $arItem["ID"] . '">' . $arItem["NAME"] . '</div>');
        if ($arItem["MODIFIED_BY"] > 0) {
            $row->AddViewField("MODIFIED_BY",
                '[<a target="_blank" href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arItem["MODIFIED_BY"] . '">' . $arItem["MODIFIED_BY"] . '</a>]&nbsp;' . $arItem["USER_NAME"]);
        } else {
            $row->AddViewField("MODIFIED_BY", '');
        }
        ...
        if ($arItem["CREATED_BY"] > 0) {
            $row->AddViewField("CREATED_USER_NAME",
                '[<a target="_blank" href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arItem["CREATED_BY"] . '">' . $arItem["CREATED_BY"] . '</a>]&nbsp;' . $arItem["CREATED_USER_NAME"]);
        } else {
            $row->AddViewField("CREATED_USER_NAME", '');
        }
        ...
        $row->AddViewField("WF_STATUS_ID",
            htmlspecialcharsbx(CIBlockElement::WF_GetStatusTitle($arItem["WF_STATUS_ID"])) . '<input type="hidden" name="n' . $arItem["ID"] . '" value="' . $arItem["NAME"] . '">');
        if ($arItem["WF_LOCKED_BY"] > 0) {
            $row->AddViewField("LOCKED_USER_NAME",
                '&nbsp;<a href="user_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $arItem["WF_LOCKED_BY"] . '" title="' . $oMessage->get("USERINFO") . '">' . $arItem["LOCKED_USER_NAME"] . '</a>');
        } else {
            $row->AddViewField("LOCKED_USER_NAME", '');
        }
        ...
        foreach ($arSelectedProps as $aProp) {
            ...
            foreach ($arProperties[$aProp['ID']] as $property_value_id => $property_value) {
                ...
                if (array_key_exists("GetAdminListViewHTML", $arUserType)) {
                    ...
                    if ($t) {
                        $res = $t['NAME'] . ' [<a href="' . htmlspecialcharsbx(CIBlock::GetAdminSectionEditLink($t['IBLOCK_ID'],
                                $t['ID'])) . '" title="' . $oMessage->get("SECTION_EDIT") . '">' . $t['ID'] . '</a>]';
                    }
                } elseif ($aProp['PROPERTY_TYPE'] == 'E') {
                    $t = GetElementName($property_value["VALUE"]);
                    if ($t) {
                        $res = $t['NAME'] . ' [<a href="' . htmlspecialcharsbx(CIBlock::GetAdminElementEditLink($t['IBLOCK_ID'],
                                $t['ID'])) . '" title="' . $oMessage->get("ELEMENT_EDIT") . '">' . $t['ID'] . '</a>]';
                    }
        ...
        $arItemsJorJs[$arItem['ID']] = array(
            ...
            'xmlTitle' => $arItem["NAME"] . ' [' . $arItem['XML_ID'] . ']',
            'name' => $arItem["NAME"],
        );
        ...
        $row->AddActions(array(
            array(
                ...
                "ACTION" => "javascript:VKapiMarketIblockElementSearchJs.selectedValue(" . $arItem['ID'] . ")",
            ),
        ));
    ...
    $lAdmin->AddFooter(
        array(
            array("title" => $oMessage->get("MAIN_ADMIN_LIST_SELECTED"), "value" => $rsData->SelectedRowsCount()),
            array("counter" => true, "title" => $oMessage->get("MAIN_ADMIN_LIST_CHECKED"), "value" => "0"),
        )
    );
...
?>
    <script type="text/javascript">
        var VKapiMarketIblockElementSearchJs = new VKapiMarketIblockElementSearch(<?= \Bitrix\Main\Web\Json::encode(array(
            'tableId' => $sTableID,
            'reloadUrl' => $reloadUrl,
            'items' => $arItemsJorJs
        ));?>);
...
?>
    <form name="form1" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>">
            ...
            if (!$iblockFix) {
                ...
                <tr>
                    ...
                    <td><? echo GetIBlockDropDownListEx(
                            ...
                            '',
                            'VKapiMarketIblockElementSearchJs.changeIblock(this)'
                        ); ?></td>
                </tr>
                <?
            }
        ?>
        <tr>
            <td><? echo $oMessage->get("FROMTO_ID") ?></td>
            <td>
        ...
        <tr>
            <td><? echo $oMessage->get("FIELD_TIMESTAMP_X") . ":" ?></td>
            <td><? echo CalendarPeriod("filter_timestamp_from", htmlspecialcharsbx($filter_timestamp_from),
                    "filter_timestamp_to", htmlspecialcharsbx($filter_timestamp_to), "form1") ?></td>
        </tr>

        <tr>
            <td><?= $oMessage->get("FIELD_MODIFIED_BY") ?>:</td>
            <td>
                <? echo FindUserID(
                    "filter_modified_user_id",
                    $filter_modified_user_id,
        ...
        <? if (is_array($arIBTYPE) && ($arIBTYPE["SECTIONS"] == "Y")): ?>
            <tr>
                <td><? echo $oMessage->get("FIELD_SECTION_ID") ?>:</td>
                <td>
                    <select name="filter_section">
                        <option value=""><? echo $oMessage->get("IBLOCK_VALUE_ANY") ?></option>
                        <option value="0"<? if ($filter_section == "0") echo " selected" ?>><? echo $oMessage->get("IBLOCK_UPPER_LEVEL") ?></option>
                        <?
                    ...
                    <input type="checkbox" name="filter_subsections"
                           value="Y"<? if ($filter_subsections == "Y") echo " checked" ?>> <? echo $oMessage->get("INCLUDING_SUBSECTIONS") ?>
        ...
        <tr>
            <td><? echo $oMessage->get("FIELD_ACTIVE") ?>:</td>
            <td>
                <select name="filter_active">
                    <option value=""><?= htmlspecialcharsbx($oMessage->get('IBLOCK_VALUE_ANY')) ?></option>
                    <option value="Y"<? if ($filter_active == "Y") echo " selected" ?>><?= htmlspecialcharsbx($oMessage->get("IBLOCK_YES")) ?></option>
                    <option value="N"<? if ($filter_active == "N") echo " selected" ?>><?= htmlspecialcharsbx($oMessage->get("IBLOCK_NO")) ?></option>
                </select>
            </td>
        </tr>
        <tr>
            <td><?= $oMessage->get("FIELD_EXTERNAL_ID") ?>:</td>
            <td><input type="text" name="filter_external_id" value="<? echo htmlspecialcharsbx($filter_external_id) ?>"
                       size="30"></td>
        </tr>
        <tr>
            <td><? echo $oMessage->get("FIELD_NAME") ?>:</td>
            <td>
                <input type="text" name="filter_name" value="<? echo htmlspecialcharsbx($filter_name) ?>" size="30">
            </td>
        </tr>
        <tr>
            <td><? echo $oMessage->get("FIELD_CODE") ?>:</td>
            <td>
                <input type="text" name="filter_code" value="<? echo htmlspecialcharsbx($filter_code) ?>" size="30">
            </td>
        </tr>
        <tr>
            <td><? echo $oMessage->get("DESC") ?></td>
            <td>
                    ...
                    <td>
                        ...
                        elseif ($prop["PROPERTY_TYPE"] == 'L'):?>
                            <select name="find_el_property_<?= $prop["ID"] ?>">
                                <option value=""><? echo $oMessage->get("IBLOCK_VALUE_ANY") ?></option><?
                                    $dbrPEnum = CIBlockPropertyEnum::GetList(array("SORT" => "ASC", "VALUE" => "ASC"),
                        ...
                        elseif ($prop["PROPERTY_TYPE"] == 'G'):
                            _ShowGroupPropertyField('find_el_property_' . $prop["ID"], $prop,
                                ${'find_el_property_' . $prop["ID"]}, $oMessage);
                        else:
                            ?>
        ...
        <span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="set_filter"
                                          value="<? echo $oMessage->get("SET_BUTTON"); ?>"
                                          title="<? echo $oMessage->get("SET_BUTTON"); ?>"
        ...
        <span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="del_filter"
                                          value="<? echo $oMessage->get("CLEAR_BUTTON"); ?>"
                                          title="<? echo $oMessage->get("CLEAR_BUTTON"); ?>"
