...
use \Arturgolubev\Gmerchant\Tools;
use \Arturgolubev\Gmerchant\Ui as DetailUi;
...
CModule::IncludeModule('arturgolubev.gmerchant');
...
$arCondSelectProp = array(
	...
	'MORE' => GetMessage('GOOGLE_FILTER_SYMBOL_MORE'),
	'LESS' => GetMessage('GOOGLE_FILTER_SYMBOL_LESS'),
);
...
$arIBlock["SALE_FIELDS"][] = 'ELEMENT_PAGE_TITLE_MAIN';
$arIBlock["SALE_FIELDS"][] = 'ELEMENT_META_TITLE_MAIN';
$arIBlock["SALE_FIELDS"][] = 'ELEMENT_META_DESCRIPTION_MAIN';
$arIBlock["SALE_FIELDS"][] = 'ELEMENT_META_KEYWORDS_MAIN';
...
$arIBlock["SALE_FIELDS"][] = 'CATALOG_QUANTITY';
$arIBlock["SALE_FIELDS"][] = 'CATALOG_MEASURE';
$arIBlock["SALE_FIELDS"][] = 'CATALOG_PURCHASING_PRICE';
$arIBlock["SALE_FIELDS"][] = 'CATALOG_PURCHASING_PRICE_FORMAT';


if($boolOffers) $arIBlock["SALE_SKU_FIELDS"] = array();
if($boolOffers) $arIBlock["SALE_SKU_FIELDS"][] = 'ID_SKU';
...
$arIBlock["FILTER_FIELDS"][] = 'CATALOG_QUANTITY';
...
if(class_exists('CCatalogStore')){
	...
	while($arStock = $dbResult->Fetch()){
		$arIBlock["FILTER_FIELDS"][] = array("FILTER"=>'CATALOG_STORE_AMOUNT_'.$arStock["ID"], "NAME"=> GetMessage("AG_GM_FILTER_NAME_CATALOG_STORE_AMOUNT").$arStock["TITLE"], "TYPE"=> GetMessage("AG_GM_FILTER_TYPE_CATALOG_STORE_AMOUNT"));
	}
}
...
$googleConfig[] = 'id';
if($boolOffers) $googleConfig[] = 'id_sku';
...
$googleConfig[] = 'mpn';
if($boolOffers) $googleConfig[] = 'mpn_sku';

$googleConfig[] = 'additional_image_link';
if($boolOffers) $googleConfig[] = 'additional_image_link_sku';

$googleConfig[] = 'availability';
if($EXPORT_TYPE == '' || $EXPORT_TYPE == 'google' || $EXPORT_TYPE == 'googlerem'){
	$googleConfig[] = 'availability_date';
}
...
$googleConfig[] = 'shipping_price';
$googleConfig[] = 'shipping_country';
if($EXPORT_TYPE == '' || $EXPORT_TYPE == 'google' || $EXPORT_TYPE == 'googlerem'){
	
	$googleConfig[] = 'product_weight';
	$googleConfig[] = 'product_length';
	$googleConfig[] = 'product_width';
	$googleConfig[] = 'product_height';
	
	$googleConfig[] = 'shipping_length';
	$googleConfig[] = 'shipping_width';
	$googleConfig[] = 'shipping_height';
	$googleConfig[] = 'shipping_label';
	$googleConfig[] = 'ships_from_country';
	$googleConfig[] = 'cost_of_goods_sold';
	$googleConfig[] = 'pickup_method';
	$googleConfig[] = 'pickup_SLA';
	$googleConfig[] = 'unit_pricing_measure';
	$googleConfig[] = 'unit_pricing_base_measure';
	$googleConfig[] = 'store_code';
	$googleConfig[] = 'region_id';
	$googleConfig[] = 'quantity';
}
...
if($EXPORT_TYPE == '' || $EXPORT_TYPE == 'facebook'){
	$googleConfig[] = 'inventory';
	$googleConfig[] = 'override';
	$googleConfig[] = 'fb_product_category';
}

if($EXPORT_TYPE == '' || $EXPORT_TYPE == 'googlerem'){
	$googleConfig[] = 'display_ads_id';
	$googleConfig[] = 'display_ads_similar_id';
	$googleConfig[] = 'display_ads_title';
	$googleConfig[] = 'display_ads_link';
	$googleConfig[] = 'display_ads_value';
	$googleConfig[] = 'excluded_destination';
	$googleConfig[] = 'ads_grouping';
	$googleConfig[] = 'ads_labels';
}
...
if ($boolOffers)
{
	$rsProps = CIBlockProperty::GetList(array('NAME'=>'ASC', 'SORT' => 'ASC'),array('IBLOCK_ID' => $intOfferIBlockID,'ACTIVE' => 'Y'));
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
		...
		if ($boolOffers)
		{
			...
			if (YANDEX_SKU_EXPORT_PROP == $arSKUExport['SKU_EXPORT_COND'])
			{
				...
				if ($boolCheck)
				{
					...
					if ($strPropCond == 'EQUAL' || $strPropCond == 'NONEQUAL' || $strPropCond == 'MORE' || $strPropCond == 'LESS')
					{
	...
	else
	{
		...
		****************************************************************************/
		...
		if (isset($arXmlData['OLD_PRICE']))
			$OLD_PRICE = $arXmlData['OLD_PRICE'];
...
?>
			...
			<td colspan="2">
				...
				<table class="inner" id="yandex_params_tbl" style="text-align: center; margin: 0 auto;">
					...
					<tbody>
						...
						{
							echo DetailUi::appendFilterRow($arIBlock, $intCount, $arParamDetail['PARAM'], '');
							$intCount++;
						}
						if ($intCount == 0)
						{
							echo DetailUi::appendFilterRow($arIBlock, $intCount, '', '');
							$intCount++;
						}
						?>
					</tbody>
	...
	$arFieldSubTitle = array(
		"utm_source" => GetMessage('ARTURGOLUBEV_GMERCHANT_UTM_TITLE'),
		"custom_label_0" => GetMessage('ARTURGOLUBEV_GMERCHANT_CUSTOM_LABEL_TITLE'),
		"shipping_weight" => GetMessage('ARTURGOLUBEV_GMERCHANT_WEIGHT_PARAM'),
		"inventory" => GetMessage('ARTURGOLUBEV_GMERCHANT_FACEBOOK_SPECIALS'),
		"display_ads_id" => GetMessage('ARTURGOLUBEV_GMERCHANT_GOOGLE_REMARKETING'),
		"store_code" => GetMessage('ARTURGOLUBEV_GMERCHANT_GOOGLE_ASSORTIMENT'),
	);

	$arFieldGoogleSupport = array(
		"id" => "6324405",
		"title" => "6324415",
		"description" => "6324468",
		"gtin" => "6324461",
		"product_highlight" => "9216100",
		"condition" => "6324469",
		"availability" => "6324448",
		"availability_date" => "6324470",
		"product_type" => "6324406",
		"additional_image_link" => "6324370",
		"brand" => "6324351",
		"mpn" => "6324482",
		"mobile_link" => "6324459",
		...
		"size_system" => "6324502",
		"size" => "6324492",
		"cost_of_goods_sold" => "9017895",
		"pickup_method" => "7177601",
		"pickup_SLA" => "7177601",
		"unit_pricing_measure" => "6324455",
		"unit_pricing_base_measure" => "6324490",
		"store_code" => "3061342",
		"region_id" => "9698880",
		"quantity" => "3061342",
	
		"product_weight" => "11018531",
		"product_length" => "11018531",
		"product_width" => "11018531",
		"product_height" => "11018531",
		
		"shipping_weight" => "6324503",
		"shipping_length" => "6324498",
		"shipping_width" => "6324498",
		"shipping_height" => "6324498",
		
		"shipping_label" => "6324504",
		"ships_from_country" => "9837936",
		...
		"utm_source" => "https://support.google.com/analytics/answer/1033863?hl=ru",
		"utm_medium" => "https://support.google.com/analytics/answer/1033863?hl=ru",
		"utm_campaign" => "https://support.google.com/analytics/answer/1033863?hl=ru",
		"utm_content" => "https://support.google.com/analytics/answer/1033863?hl=ru",
		"utm_term" => "https://support.google.com/analytics/answer/1033863?hl=ru",
		
		"display_ads_id" => "6069387",
		"display_ads_similar_id" => "6069387",
		"display_ads_title" => "6069387",
		"display_ads_link" => "6069387",
		"display_ads_value" => "6069387",
		"excluded_destination" => "6069387",
		"ads_grouping" => "6069387",
		"ads_labels" => "6069387",
		...
		"price" => "6324371",
		"sale_price" => "6324471",
		"sale_price_effective_date" => "6324460",
		"identifier_exists" => "6324478",
	);
	...
	<tr>
		<td colspan="2" style="border-bottom: 1px solid #ddd;"><?=GetMessage('ARTURGOLUBEV_GMERCHANT_BEFORE_FIELDS_TEXT')?></td>
	</tr>
	...
	<tr>
		<td colspan="2">
			<?foreach ($arTypesConfig as $key => $arConfig):?>
				<div id="config_<?=htmlspecialcharsbx($key)?>" style="display: <? echo ($type == $key ? 'block' : 'none'); ?>;">
					<table width="90%" class="inner detail-prop-table" style="text-align: center;">
						<?foreach ($arConfig as $kprop=>$prop):
							if(strstr($prop, '_sku')) continue;
						
							// $doptitle = '';
							// if(strstr($arConfig[$kprop], '_sku'))
								// $doptitle = GetMessage('ARTURGOLUBEV_GMERCHANT_SKU_PRODUCT_SUBTEXT');
							// if(strstr($arConfig[($kprop+1)], '_sku'))
								// $doptitle = GetMessage('ARTURGOLUBEV_GMERCHANT_MAIN_PRODUCT_SUBTEXT');

							$clear_prop = str_replace(array('_sku', '_alternative_1', '_alternative_2', '_alternative_3', '_alternative_4'), '', $prop);
							
							$skuKey = (strstr($arConfig[$kprop+1], '_sku')) ? $arConfig[$kprop+1] : false;
						?>
							<?if($arFieldSubTitle[$prop]):?>
								<tr class="heading">
									<td colspan="3"><?=$arFieldSubTitle[$prop]?></td>
								</tr>
							<?endif;?>
							<tr>
								<td align="right" valign="middle" style="min-width: 320px;">
									<?=(GetMessage('ARTURGOLUBEV_GMERCHANT_'.$prop))?><?//=$doptitle?>:
									<br>
									<small>(<?
									echo $clear_prop;
									if($arFieldGoogleSupport[$clear_prop])
									{
										if(strstr($arFieldGoogleSupport[$clear_prop], 'http')){
											echo ' - <a href="'.$arFieldGoogleSupport[$clear_prop].'" target="_blank">'.GetMessage("ARTURGOLUBEV_GMERCHANT_GOOGLE_SUPPORT_HREF").'</a>';
										}else{
											echo ' - <a href="https://support.google.com/merchants/answer/'.$arFieldGoogleSupport[$clear_prop].'?hl=ru" target="_blank">'.GetMessage("ARTURGOLUBEV_GMERCHANT_GOOGLE_SUPPORT_HREF").'</a>';
										}
									}
									?>)</small>
								</td>
								<td style="white-space: nowrap;" <?=(!$skuKey)?'colspan=2':'';?>>
									<?if($skuKey):?>
										<?=GetMessage('ARTURGOLUBEV_GMERCHANT_MAIN_PRODUCT_SUBTEXT')?><br>
									<?endif;?>
									<?DetailUi::showRelationSelect($key, $prop, $arIBlock, (isset($arTypeValues[$key][$prop]) ? $arTypeValues[$key][$prop] : ''), GetMessage('ARTURGOLUBEV_GMERCHANT_'.$prop."_EMPTY"), $skuKey)?>
									<br>
									<?DetailUi::showRelationInput($key, $prop, $arIBlock, (isset($arTypeValues[$key][$prop]) ? $arTypeValues[$key][$prop] : ''), $arXmlData["TEXT_FIELDS"][$key][$prop]);?>
								</td> 
								
								<?if($skuKey):?>
									<td style="white-space: nowrap;">
										<?=GetMessage('ARTURGOLUBEV_GMERCHANT_SKU_PRODUCT_SUBTEXT')?><br>
										<?DetailUi::showRelationSelect($key, $skuKey, $arIBlock, (isset($arTypeValues[$key][$skuKey]) ? $arTypeValues[$key][$skuKey] : ''), GetMessage('ARTURGOLUBEV_GMERCHANT_'.$skuKey."_EMPTY"), 0)?>
										<br>
										<?DetailUi::showRelationInput($key, $skuKey, $arIBlock, (isset($arTypeValues[$key][$skuKey]) ? $arTypeValues[$key][$skuKey] : ''), $arXmlData["TEXT_FIELDS"][$key][$skuKey]);?>
									</td>
								<?endif;?>
							</tr>
						<?endforeach;?>
						...
						<tr class="heading">
							<td colspan="3"><?=GetMessage("ARTURGOLUBEV_GMERCHANT_AUTO_FIELDS");?></td>
						</tr>
						
						<?foreach($autoFieldsList as $prop):?>
							<tr>
								<td align="right" valign="middle">
									<?=(GetMessage('ARTURGOLUBEV_GMERCHANT_'.$prop))?>:
									<br>
									<small>(<?
									echo $prop;

									if($arFieldGoogleSupport[$prop])
									{
										if(strstr($arFieldGoogleSupport[$prop], 'http')){
											echo ' - <a href="'.$arFieldGoogleSupport[$prop].'" target="_blank">'.GetMessage("ARTURGOLUBEV_GMERCHANT_GOOGLE_SUPPORT_HREF").'</a>';
										}else{
											echo ' - <a href="https://support.google.com/merchants/answer/'.$arFieldGoogleSupport[$prop].'?hl=ru" target="_blank">'.GetMessage("ARTURGOLUBEV_GMERCHANT_GOOGLE_SUPPORT_HREF").'</a>';
										}
									}
									?>)</small>
								</td>
								<td colspan="2">
									<div><?=GetMessage('ARTURGOLUBEV_GMERCHANT_'.$prop."_EMPTY")?>
								</td>
							</tr>
						<?endforeach;?>
					</table>
...
<?$tabControl->BeginNextTab(); // price and filter?>
	...
	<tr>
		<td colspan="2"><?=GetMessage('AG_GM_PRICE_FILTER_INCLUDE_TEXT')?>:
			<span class="" style="display: inline-block; width: 25px;"><?=GetMessage('AG_GM_PRICE_FILTER_INCLUDE_TEXT_FROM')?></span><input type="text" name="F_PRICE_FROM" value="<?=$F_PRICE_FROM?>" />
			<span class="" style="display: inline-block; width: 25px;"><?=GetMessage('AG_GM_PRICE_FILTER_INCLUDE_TEXT_TO')?></span><input type="text" name="F_PRICE_TO" value="<?=($F_PRICE_TO) ? $F_PRICE_TO : ''?>" />
			<br>
		...
		<td colspan="2">
			<div id="config_param" style="padding: 10px auto; text-align: center;">
				<table class="inner" id="yandex_params_tbl" style="text-align: center; margin: 0 auto;">
					<thead>
					<tr>
						<td style="text-align: left; padding-bottom: 8px; font-weight: bold;"><? echo GetMessage('GOOGLE_FILTER_FIELD_PARAM_TITLE'); ?></td>
						<td style="text-align: left; padding-bottom: 8px; font-weight: bold;"><? echo GetMessage('GOOGLE_FILTER_FIELD_PARAM_SYMBOL'); ?></td>
				...
				<input type="hidden" name="PARAMS_COUNT" id="PARAMS_COUNT" value="<? echo $intCount; ?>">
				<div style="width: 100%; text-align: center; margin-top: 5px;"><input type="button" onclick="__addYP(); return false;" name="yandex_params_add" value="<? echo GetMessage('GOOGLE_FILTER_ADD_MORE'); ?>"></div>
				<br>
				
				<div style="text-align: left; margin-bottom: 10px;"><a href="#" onClick="toggleElementView('productFilterDescription'); return false" class="adm-toggler-title"><?=GetMessage("GOOGLE_FILTER_DESCRIPTION_TITLE");?></a></div>
					
				<div class="adm-info-message-wrap adm-toggler-body" id="productFilterDescription" style="display:none;">
					<div class="adm-info-message">
						<?=GetMessage("GOOGLE_FILTER_DESCRIPTION");?>
						<?=GetMessage("ARTURGOLUBEV_GMERCHANT_FILTER_EVENTS_HELP");?>
			...
			<script type="text/javascript">
			function toggleElementView(eid){
			...
			function __addYP()
			{
				...
				strContent = '<? echo CUtil::JSEscape(DetailUi::_filterRowColName($arIBlock, 'tmp_xxx', '')); ?>';
				strContent = strContent.replace(/tmp_xxx/ig, id);
				oCell.innerHTML = strContent;
				...
				oCell.style.padding = '5px 0';
				oCell.style.verticalAlign = 'top';
				strContent = '<? echo CUtil::JSEscape(DetailUi::_filterRowColSymbol($arIBlock, 'tmp_xxx', '')); ?>';
				strContent = strContent.replace(/tmp_xxx/ig, id);
				oCell.innerHTML = strContent;
				...
				oCell.style.padding = '5px 0';
				oCell.style.verticalAlign = 'top';
				strContent = '<? echo CUtil::JSEscape(DetailUi::_filterRowColValue($arIBlock, 'tmp_xxx', '')); ?>';
				strContent = strContent.replace(/tmp_xxx/ig, id);
				oCell.innerHTML = strContent;
			}
	...
	<?if ($boolOffers):?>
		...
		<tr>
		<td valign="middle"><? echo GetMessage('YANDEX_OFFERS_SELECT') ?></td>
		<td valign="middle"><?
		...
		// if(!$arSKUExport['SKU_EXPORT_COND']) $arSKUExport['SKU_EXPORT_COND'] = YANDEX_SKU_EXPORT_ALL;
		...
		{			
			...
			<div class="" style="margin-top:10px;"><?=GetMessage("ARTURGOLUBEV_GMERCHANT_FILTER_EVENTS_HELP")?></div>
			</div><?
		}
		?></td>
		</tr>
...
<tr>
	...
	<td>
		<select name="PRICE">
			...
			<optgroup label="<?=GetMessage("AG_GM_SELECT_GROUP_PRICES")?>">
				<?
...
<tr>
	...
	<td>
		<div class="field-annotation"><?=GetMessage("AG_GM_PRICE_ANNOTATION");?></div>
	</td>
</tr>
<tr>
	<td><?=GetMessage('AG_GM_OLD_PRICE_TYPE');?>: </td>
	<td>
		<select name="OLD_PRICE">
		<option value=""<? echo ($OLD_PRICE == "" || $OLD_PRICE == 0 ? ' selected' : '');?>><?=GetMessage('YANDEX_OLD_PRICE_TYPE_NONE');?></option>
		...
		if(count($tmp)):?>
			<optgroup label="<?=GetMessage("AG_GM_SELECT_GROUP_PRICES")?>">
				<?
				foreach($tmp as $arRes){
					?>
					<option value="<?= $arRes['ID'] ?>"<? echo($OLD_PRICE == $arRes['ID'] ? ' selected' : ''); ?>><?= '[' . $arRes['ID'] . '] ' . htmlspecialcharsEx($arRes['NAME']); ?></option><?
				}
				?>
			</optgroup>
		<?endif;?>
			...
			<?
			$tmp = array();
			foreach($arIBlock['PROPERTY'] as $k=>$v){
				if(!$v['CODE'] || $v["PROPERTY_TYPE"] != 'N') continue;
				$tmp[] = $v;
			}
			if(count($tmp)):?>
				<optgroup label="<?=GetMessage("AG_GM_SELECT_GROUP_PROPERTIES")?>">
					<?
					foreach($tmp as $v){
						?>
						<option value="<?=$v['CODE']?>"<? echo(strval($OLD_PRICE) == strval($v['CODE']) ? ' selected' : ''); ?>><?= '[' . $v['CODE'] . '] ' . htmlspecialcharsEx($v['NAME']); ?></option><?
					}
					?>
				</optgroup>
			<?endif;?>
...
<tr>
	...
	<td>
		<div class="field-annotation"><?=GetMessage("AG_GM_OLD_PRICE_ANNOTATION");?></div>
	</td>
</tr>
...
if(CheckVersion($saleModuleInfo->MODULE_VERSION, '17.5.0')):
	$arCurrencyList = Tools::getCurrencyList();
	?>
	<tr>
