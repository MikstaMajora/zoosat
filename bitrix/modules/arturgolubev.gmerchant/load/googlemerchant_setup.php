...
use \Arturgolubev\Gmerchant\Simplefilter;
...
$sectionFramePath = '/bitrix/tools/arturgolubev.gmerchant/google_util.php';
if(!file_exists($_SERVER["DOCUMENT_ROOT"].$sectionFramePath))
{
	// fix old version bug
	$sectionFramePath = '/bitrix/tools/catalog_export/yandex_util.php';
}
...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
	...
	if (isset($arOldSetupVars['NO_CLEAR_DESCRIPTION_TAGS']))
		$NO_CLEAR_DESCRIPTION_TAGS = $arOldSetupVars['NO_CLEAR_DESCRIPTION_TAGS'];
	...
	if (isset($arOldSetupVars['EXPORT_TYPE']))
		$EXPORT_TYPE = $arOldSetupVars['EXPORT_TYPE'];
...
if ($STEP > 1)
{
	...
	if (CCatalogSku::TYPE_PRODUCT == $arCatalog['CATALOG_TYPE'] || CCatalogSku::TYPE_FULL == $arCatalog['CATALOG_TYPE'])
	{
		...
		{
			$arSetupErrors[] = GetMessage('ARTURGOLUBEV_GMERCHANT_ERR_SKU_SETTINGS_ABSENT');
		}
	}
...
<?CJSCore::Init(array("aggm_select2"));?>

<tr class="heading">
	<td colspan="2"><?=GetMessage('BX_CATALOG_EXPORT_IBLOCK_SELECT'); ?></td>
</tr>
<tr>
	<td width="40%"><?=GetMessage('ARTURGOLUBEV_GMERCHANT_SELECT_IBLOCK');?></td>
	<td width="60%"><?
	echo GetIBlockDropDownListEx(
		...
		"ClearSelected(); changeIblockSites(0); BX('id_ifr').src='".$sectionFramePath."?IBLOCK_ID=0&'+'".bitrix_sessid_get()."';",
		"ClearSelected(); changeIblockSites(this[this.selectedIndex].value); BX('id_ifr').src='".$sectionFramePath."?IBLOCK_ID='+this[this.selectedIndex].value+'&'+'".bitrix_sessid_get()."';",
...
<tr>
	<td width="40%" valign="top"><?echo GetMessage("ARTURGOLUBEV_GMERCHANT_SELECT_GROUP");?></td>
	<td width="60%"><?
	if ($intCountSelected)
	{
		foreach ($V as $oneKey)
	...
	function checkSelected(checked, i){
		if (!checked){
			var r = checkInnerChecks(i);
			if(r) return ' (<?echo GetMessageJS("ARTURGOLUBEV_GMERCHANT_SECTION_INNER_SELECTED");?>)';
		}
		
		return '';
	}
	
	function checkInnerChecks(node){
		var res = 0;
		
		for (i in Tree[node])
		{
			if(BX.util.in_array(i,TreeSelected)){
				res++;
			}
			else
			{
				res = res + checkInnerChecks(i);
			}
		}
	...
	function buildMenu()
	{
		...
		for (i in Tree[0])
		{
			...
			{
				space = '<input type="checkbox" name="V[]" value="'+i+'"'+(BX.util.in_array(i,TreeSelected) ? ' checked' : '')+' onclick="delOldV(this);"><a href="javascript: collapse(' + i + ')"><span class="text"><b>' + Tree[0][i][0] + '</b>'+checkSelected(BX.util.in_array(i,TreeSelected), i)+'</span></a>';
	...
	function collapse(node)
	{
		if (!BX('table_' + node))
		{
			...
			for (i in Tree[node])
			{
				...
				{
					space = '<input type="checkbox" name="V[]" value="'+i+'"'+(BX.util.in_array(i,TreeSelected) ? ' checked' : '')+' onclick="delOldV(this);"><a href="javascript: collapse(' + i + ')"><font class="text"><b>' + Tree[node][i][0] + '</b>'+checkSelected(BX.util.in_array(i,TreeSelected), i)+'</font></a>';
					imgSpace = '<img src="/bitrix/images/catalog/load/plus.gif" width="13" height="13" id="img_' + i + '" OnClick="collapse(' + i + ')">';
				}
	...
	<iframe src="<?=$sectionFramePath?>?IBLOCK_ID=<?=intval($IBLOCK_ID)?>&<? echo bitrix_sessid_get(); ?>" id="id_ifr" name="ifr" style="display:none"></iframe>
	</td>
</tr>

<?
$arExportTypes = array(
	"" => GetMessage('ARTURGOLUBEV_GMERCHANT_EXPORT_TYPE_NONE'),
	"tiktok" => GetMessage('ARTURGOLUBEV_GMERCHANT_EXPORT_TYPE_TIKTOK'),
	"facebook" => GetMessage('ARTURGOLUBEV_GMERCHANT_EXPORT_TYPE_FACEBOOK'),
	"google" => GetMessage('ARTURGOLUBEV_GMERCHANT_EXPORT_TYPE_GOOGLE_MERCHANT'),
	"googlerem" => GetMessage('ARTURGOLUBEV_GMERCHANT_EXPORT_TYPE_GOOGLE_MERCHANT_DYNAMIC'),
);
?>
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_EXPORT_TYPE'); ?></td>
	<td width="60%">
		<select name="EXPORT_TYPE" id="EXPORT_TYPE">
			<?foreach($arExportTypes as $k=>$v):?>
				<option value="<?=$k?>" <?if($k && $k == $EXPORT_TYPE) echo 'selected';?>><?=$v?></option>
			<?endforeach;?>
		</select>
	</td>
</tr>
		...
		function showDetailPopup()
		{
			if (!obDetailWindow)
			{
				...
				var obDetailWindow = new BX.CAdminDialog({
					'content_url': '/bitrix/tools/arturgolubev.gmerchant/google_detail.php?lang=<?=LANGUAGE_ID?>&bxpublic=Y&IBLOCK_ID=' + s[s.selectedIndex].value+'&EXPORT_TYPE='+ev,
...
<?/* <tr>
	...
	<td width="60%">
		<?
		Simplefilter::preInit();
		?>
	</td>
</tr> */?>
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_CHECK_PERMISSIONS'); ?></td>
	<td width="60%">
...
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_FILTER_AVAILABLE'); ?></td>
	<td width="60%">
...
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_HIDE_WITHOT_PICTURES'); ?></td>
	<td width="60%">
...
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_HIDE_WITHOT_DESCRIPTION'); ?></td>
	<td width="60%">
...
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_HIDE_QUANTITY_NULL'); ?></td>
	<td width="60%">
...
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_NO_CLEAR_DESCRIPTION_TAGS'); ?></td>
	<td width="60%">
		<input type="text" name="NO_CLEAR_DESCRIPTION_TAGS" value="<?=$NO_CLEAR_DESCRIPTION_TAGS?>" size="50">
	</td>
</tr>
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_NO_USE_STANDART_PICTURES'); ?></td>
	<td width="60%">
...
<tr>
	<td colspan="2">
		<div class="adm-info-message-wrap"><div class="adm-info-message simple-hint"><?echo GetMessage('ARTURGOLUBEV_GMERCHANT_NO_USE_STANDART_PICTURES_NOTE');?></div>		</div>
	</td>
</tr>

<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_ONLY_STANDART_PRICE'); ?></td>
	<td width="60%">
...
<tr>
	<td colspan="2">
		<div class="adm-info-message-wrap"><div class="adm-info-message simple-hint"><?echo GetMessage('ARTURGOLUBEV_GMERCHANT_ONLY_STANDART_PRICE_NOTE');?></div></div>
	</td>
</tr>
...
<tr>
	<td width="40%"><? echo GetMessage('ARTURGOLUBEV_GMERCHANT_LOCK_CUPON_CHECK'); ?></td>
	<td width="60%">
...
<tr>
	<td colspan="2">
		<div class="adm-info-message-wrap"><div class="adm-info-message simple-hint"><?echo GetMessage('ARTURGOLUBEV_GMERCHANT_LOCK_CUPON_CHECK_NOTE');?></div></div>
	</td>
</tr>
...
foreach (GetModuleEvents($module_id, "onAfterPrepareProductFilter", true) as $arEvent){
	$arModuleEvents[] = $arEvent;
}
foreach (GetModuleEvents($module_id, "onAfterPrepareSkuFilter", true) as $arEvent){
	$arModuleEvents[] = $arEvent;
}
?>
<?if(!empty($arModuleEvents)):?>
	<div class="adm-info-message-wrap" style="width:100%;box-sizing:border-box;">
		<div class="adm-info-message" style="width:100%;box-sizing:border-box;background:#fff;border-color:#a4b9cc;">
			<div class="adm-info-message-title" style="font-size: 16px;"><?=GetMessage("ARTURGOLUBEV_GMERCHANT_CALLBACKS_TITLE")?></div><br>
			<div class="adm-info-message-text">
				<?foreach($arModuleEvents as $arEvent):?>
					<div class="">
					<?if(is_array($arEvent["CALLBACK"])):?>
						<?=implode("::", $arEvent["CALLBACK"])?>
					<?else:?>
						<?=$arEvent["CALLBACK"]?>
					<?endif;?>
					</div>
...
<div class="adm-info-message-wrap" style="width:100%;box-sizing:border-box;">
	<div class="adm-info-message" style="width:100%;box-sizing:border-box;background:#fff;border-color:#a4b9cc;">
		<div class="adm-info-message-title" style="font-size: 16px;"><?=GetMessage("ARTURGOLUBEV_GMERCHANT_INFORMATION_TITLE")?></div><br>
...
<style>
	.detail-prop-table tr:hover {
		background-color: #dce0e0;
	}
	.detail-prop-table tr td {
		padding: 8px 10px;
	}
	.detail-prop-table select, .detail-prop-table input[type=text] {
		width:320px;
		box-sizing: border-box;
	}
	.detail-prop-table a {
