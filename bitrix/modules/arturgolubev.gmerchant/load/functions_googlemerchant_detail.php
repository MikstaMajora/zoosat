<?
// no actual use, for compatibility
if (!function_exists("__yand_show_selector"))
{
	...
	function appendFilterRow(&$IBLOCK, $intCount, $strParam, $strUnit)
	{
		...
		function __addParamNameFilter(&$IBLOCK, $intCount, $value)
		{
			...
			function __yand_show_selector_filter($group, $key, $IBLOCK, $valueSaved = "")
			{
				...
				<?if(count($IBLOCK['FILTER_FIELDS'])):?>
					<option value=""><?=GetMessage("GOOGLE_FILTER_ELEMENTS_FIELDS");?></option>
					<?
				...
				<option value=""><?=GetMessage("GOOGLE_FILTER_ELEMENTS_PROPS");?></option>
				<?
				foreach ($IBLOCK['PROPERTY'] as $key => $arProp)
				{
