...
if(WIZARD_INSTALL_DEMO_DATA){
	...
	$arUserFields = array("UF_MEGA_MENU_LINK", "UF_CATALOG_ICON");
	foreach($arUserFields as $userField){
		$arLabelNames = $arLabelHelps = Array();
		WizardServices::IncludeServiceLang("megamenu.php", 'ru');

		foreach($arLanguages as $languageID){
			$arLabelNames[$languageID] = GetMessage($userField.'_'.$languageID);
			$arLabelHelps[$languageID] = GetMessage($userField.'_HELP_'.$languageID);
		}

		$arProperty = array();
		$arProperty["EDIT_FORM_LABEL"] = $arProperty["LIST_COLUMN_LABEL"] = $arProperty["LIST_FILTER_LABEL"] = $arProperty["ERROR_MESSAGE"] = $arLabelNames;
		$arProperty["HELP_MESSAGE"] = $arLabelHelps;
	...
	CUserOptions::SetOption("form", "form_section_".$iblockID, array(
		"tabs" => 'edit1--#--'.GetMessage("WZD_OPTION_299").'--,--ID--#--ID--,--ACTIVE--#--'.GetMessage("WZD_OPTION_376").'--,--IBLOCK_SECTION_ID--#-'.GetMessage("WZD_OPTION_377").'--,--XML_ID--#--'.GetMessage("WZD_OPTION_218").'--,--SORT--#--'.GetMessage("WZD_OPTION_44").'--,--NAME--#--'.GetMessage("WZD_OPTION_54").'--,--UF_MEGA_MENU_LINK--#--'.GetMessage("WZD_OPTION_216").'--,--PICTURE--#--'.GetMessage("WZD_OPTION_10").'--,--UF_CATALOG_ICON--#--'.GetMessage("WZD_OPTION_378").'--;--',
	), true);

	// list user options
	CUserOptions::SetOption("list", "tbl_iblock_list_".md5($iblockTYPE.".".$iblockID), array(
		'columns' => '', 'by' => 'sort', 'order' => 'asc', 'page_size' => '20',
	));
}

if($iblockID){
	// replace macros IBLOCK_TYPE & IBLOCK_ID & IBLOCK_CODE
	CWizardUtil::ReplaceMacrosRecursive($wizardSitePath, Array("IBLOCK_MEGAMENU_TYPE" => $iblockTYPE));
	CWizardUtil::ReplaceMacrosRecursive($wizardSitePath, Array("IBLOCK_MEGAMENU_ID" => $iblockID));
	CWizardUtil::ReplaceMacrosRecursive($wizardSitePath, Array("IBLOCK_MEGAMENU_CODE" => $iblockCODE));
