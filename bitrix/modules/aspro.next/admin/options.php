...
if($RIGHT >= "R"){
				...
				foreach($moduleClass::$arParametrsList as $blockCode => $arBlock){
					...
					foreach($arBlock["OPTIONS"] as $optionCode => $arOption){
						if($arOption['TYPE'] === 'array'){
							...
							{
								if(isset($_REQUEST[$optionCode."_".$optionsSiteID])){
									$newVal = $_REQUEST[$optionCode."_".$optionsSiteID];
								}
								else{
									if(
										$optionCode === 'ONECLICKBUY_PROPERTIES' ||
										$optionCode === 'ONECLICKBUY_REQUIRED_PROPERTIES'
									){
										$newVal = array();
									}
									else{
	...
	<?else:?>
		...
		$dbRes = CSaleOrderProps::GetList(array('SORT' => 'ASC'), array('ACTIVE' => 'Y'), false, false, array('ID', 'CODE', 'NAME', 'PERSON_TYPE_ID', 'TYPE', 'IS_PHONE', 'IS_EMAIL', 'SORT', 'ACTIVE', 'IS_PAYER'));
		while($arItem = $dbRes->Fetch()){
			...
			<script type="text/javascript">
				$(document).ready(function() {
								...
								for(var i in arSelects){
									...
									if($fields.length){
										...
										for(var j in arOrderPropertiesByPerson[value]){
											var selected = (fields && fields.indexOf(j) !== -1 ? ' selected="selected"' : '');
					...
					$('select[name^="ONECLICKBUY_PROPERTIES"]').change(function() {
						...
						var $requiredFieldsOptions = $requiredFields.find('option');
						$requiredFieldsOptions.each(function(i){
							$option = $requiredFieldsOptions.eq(i);
							var bDisabled = fiedsValue === null || fiedsValue.indexOf($option.attr('value')) === -1;
							$option.prop('disabled', bDisabled);
					...
					$('select[name^="ONECLICKBUY_PROPERTIES"]').change();
