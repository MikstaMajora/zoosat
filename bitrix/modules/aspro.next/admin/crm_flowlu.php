	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				...
				echo \Aspro\Functions\CAsproNextCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array());
			}
			else
			{
				$arError = array(
		...
		elseif($SendCrm)
		{
			...
			{
				echo \Aspro\Functions\CAsproNext::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'FLOWLU', $_POST["SITE_ID"], false, false);
			}
			else
			{
				$arError = array(
		...
		else
		{
			foreach($arTabs as $key => $arTab)
			{
				...
				if($domain && $token)
				{
					...
					$result = json_decode(\Aspro\Functions\CAsproNextCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array()), true);
