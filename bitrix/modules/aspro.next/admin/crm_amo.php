...
if($RIGHT >= "R"){
	...
	$arTabsForView = COption::GetOptionString($moduleID, 'TABS_FOR_VIEW_ASPRO_NEXT', '');
	if($arTabsForView){
						...
						"ITEMS" => array(
							...
							"INTEGRATION_NOTE_AMO_CRM" => array(
								...
								"TITLE" => Loc::getMessage(
									"ASPRO_NEXT_MODULE_INTEGRATION_NOTE_AMO_CRM",
									array(
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$action) && $RIGHT >= "W" && check_bitrix_sessid()){
		...
		if($action === 'checkAuth'){
			...
			if(strlen($arConfig['domain'])){
				// check auth
				$arResponse = CAsproNextCRM::checkOAuth($arOAuth, $arConfig);
				if(
					$arResponse &&
					is_array($arResponse) &&
					!$arResponse['error']
					...
					CAsproNextCRM::save($arOAuth, $arConfig);
				}
			}
			else{
				$arResponse = array(
		...
		elseif($action === 'sendCrm'){
			...
			if(
				...
				echo \Aspro\Functions\CAsproNext::sendLeadCrmFromForm($formId, $resultId, 'AMO_CRM', $siteId, false, false);
			}
			else{
				$arResponse = array(
					'error' => 'empty fields',
		...
		// send statistics
		// if(\Aspro\Next\GS::isEnabled()){
		// 	if(\Aspro\Next\GS::register()){
		// 		\Aspro\Next\GS::sendData(
		// 			\Aspro\Next\GS::mkData(array('sites', 'options'))
		// 		);
		// 	}
		// }
	...
	<?if(!count($arTabs)):?>
		<div class="adm-info-message-wrap adm-info-message-red">
			...
			<a href="aspro.next_options_tabs.php" id="tabs_settings" target="_blank">
				<span>
								...
								<?if($arOption["VALUES"]):?>
									<tr>
										<td colspan="2">
											...
											if($bOAuthChecked){
												$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $optionsSiteID), CAsproNextCRM::AMO_CRM_PATH);
												$result_text = CAsproNextCRM::query($url, "/private/api/v2/json/accounts/current/", array(), $arHeaders, false, false);
															...
															if($codeGroup != "customers")
															{
																foreach($arGroup as $arProp)
																{
																	...
																	if(isset($arProp["enums"]))
																	{
																		...
																		{
																			CAsproNextCRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$keyEnum."_".$codeGroup] = (Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) ? Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) : $arProp["name"]."_".$enum);
																		}
																	}
																	else
																	{
																		CAsproNextCRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$codeGroup] = $arProp["name"];
																	}
																}
															}
														}
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							...
																							<?foreach(CAsproNextCRM::$arCrmFileds["AMO_CRM"] as $groupCode => $arGroup):?>
																					...
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							<option value=""><?=Loc::getMessage('FORM_FIELD_CRM_FIELDS_FORM_NAME_NO')?></option>
																							<optgroup label="...">
																								<?foreach(CAsproNextCRM::$arCrmFileds["MAIN"] as $key2 => $text):?>
		...
		if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<script type="text/javascript">
				BX.message({
					...
					"ASPRO_NEXT_MODULE_LINK_COPY": "<?=Loc::getMessage("ASPRO_NEXT_MODULE_LINK_COPY")?>",
					"ASPRO_NEXT_MODULE_LINK_COPYED": "<?=Loc::getMessage("ASPRO_NEXT_MODULE_LINK_COPYED")?>",
					"ASPRO_NEXT_MODULE_LINK_COPY_ERROR": "<?=Loc::getMessage("ASPRO_NEXT_MODULE_LINK_COPY_ERROR")?>",
				});
				...
				$(document).ready(function(){
					...
					if($btnCopyLink.length){
						...
						clipboard.on('success', function(e){
						    $btnCopyLink.html('<i class="fa fa-check"></i>' + BX.message('ASPRO_NEXT_MODULE_LINK_COPYED'))
							...
							copyLinkTimeout = setTimeout(function(){
								$btnCopyLink.html('<i class="fa fa-clipboard"></i>' + BX.message('ASPRO_NEXT_MODULE_LINK_COPY'))
							}, 2000)
						...
						clipboard.on('error', function(e){
						    alert(BX.message('ASPRO_NEXT_MODULE_LINK_COPY_ERROR') + $btnCopyLink.attr('data-clipboard-text'))
						});
					}
