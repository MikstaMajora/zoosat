...
class CNextEvents{
	...
	function OnSaleComponentOrderProperties(&$arUserResult, $arRequest, $arParams, $arResult){
		...
		{
			$useProfile = isset($arUserResult["PROFILE_ID"]) && (int) $arUserResult["PROFILE_ID"] > 0;
	...
	static function onAfterAjaxResponseHandler() {
		...
		CNext::setRegionSeoMarks();		

		$APPLICATION->arAdditionalChain = array_map(
			function($arElement) {
				$arElement['TITLE'] = self::replaceMarks($arElement['TITLE'], CNextRegionality::$arSeoMarks);
				return $arElement;
			}
		, $APPLICATION->arAdditionalChain);

		$APPLICATION->sDocTitle = self::replaceMarks($APPLICATION->sDocTitle, CNextRegionality::$arSeoMarks);
	}
	...
	static function OnEndBufferContentHandler(&$content)
	{
		...
		if(!defined('ADMIN_SECTION') && !defined('WIZARD_SITE_ID') && !defined('CUSTOM_CONTENT') || $bCompSaleOrderAjaxPost)
		{
			...
			if(preg_match_all('/<\s*link\s+[^\>]*rel\s*=\s*[\'"](canonical|next|prev)[\'"][^\>]*>/i'.BX_UTF_PCRE_MODIFIER, $content, $arMatches)){
				...
				// fix composite frame area buffered content
				if(class_exists('\Bitrix\Main\Composite\BufferArea')){
					if($area = \Bitrix\Main\Composite\BufferArea::getDynamicArea('viewtype-block')){
						if($stub = $area->getStub()){
							if($stub = preg_replace('/<\s*link\s+[^\>]*rel\s*=\s*[\'"](canonical|next|prev)[\'"][^\>]*>/i'.BX_UTF_PCRE_MODIFIER, '', $stub)){
								$area->setStub($stub);
							}
						}
					}
			...
			// lazyload
			if ((defined('SITE_TEMPLATE_PATH') && file_exists($_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH.'/vendor/js/lazysizes.min.js'))
				&& $GLOBALS['_USE_LAZY_LOAD_ASPRO_NEXT_'] && !CNext::checkMask(Option::get('aspro.next', 'LAZY_LOAD_EXCEPTIONS', ''))) {
				...
				if(isset($tmpContent) && (strpos($_SERVER['REQUEST_URI'], '/bitrix/components/') === false && strpos($_SERVER['REQUEST_URI'], '/bitrix/tools/') === false && strpos($_SERVER['REQUEST_URI'], '/bitrix/admin/') === false)){
					$content = $tmpContent;
					...
					$content = preg_replace('/(<img data-lazyload [^>]*)class=([\'"])/i'.BX_UTF_PCRE_MODIFIER, '${1}class=${2}lazyload ', $content);
					$content = preg_replace('/<img data-lazyload (?![^>]*\bclass\s*=\s*[\'\"]\b)/i'.BX_UTF_PCRE_MODIFIER, '${0}class="lazyload" ', $content);
				}
				else{
					// preg error
				}
			...
			if($SECTION_BNR_CONTENT)
			{
				...
				if($start>0)
				{
					...
					if(($end>0) && ($end>$start))
					{
						if(defined("BX_UTF") && BX_UTF === true && (!CNext::checkVersionModule('20.100.0', 'main')) )
			...
			if ($arRegion['LIST_STORES'] && reset($arRegion['LIST_STORES']) != 'component') {
				...
				if ($arProduct['QUANTITY_TRACE'] == 'Y' && $arProduct['CAN_BUY_ZERO'] != 'Y') {
					...
					if ($quantity_stores <= 0) {
						$arFields['QUANTITY'] = 0;
					} else if ($arFields['QUANTITY'] > $quantity_stores) {
	...
	function OnGetOptimalPriceHandler($intProductID, $quantity = 1, $arUserGroups = array(), $renewal = "N", $priceList = array(), $siteID = false, $arDiscountCoupons = false){
		...
		if($arRegion)
		{
			...
			if($arPricesID === NULL)
			{
				...
				{
					$arPricesID = array_column($arRegion['LIST_PRICES'], 'ID');
				...
				if (!$arPricesID) {
					...
					foreach ($arPricesID as $key => $id) {
						if (!$priceTypeCache[$cacheKey][$id]) {
							unset($arPricesID[$key]);
						}
					}
				}
			}
	...
	static function onBeforeResultAddHandler($WEB_FORM_ID, &$arFields, &$arrVALUES){
		if(!defined('ADMIN_SECTION') && isset($_REQUEST['aspro_next_form_validate']))
		{
			global $APPLICATION;
	...
	static function onAfterResultAddHandler($WEB_FORM_ID, $RESULT_ID){
		if(Option::get(self::moduleID, 'AUTOMATE_SEND_FLOWLU', 'Y') == 'Y')
			\Aspro\Functions\CAsproNext::sendLeadCrmFromForm($WEB_FORM_ID, $RESULT_ID, 'FLOWLU', SITE_ID, false, false);
		if(Option::get(self::moduleID, 'AUTOMATE_SEND_AMO_CRM', 'Y') == 'Y')
			\Aspro\Functions\CAsproNext::sendLeadCrmFromForm($WEB_FORM_ID, $RESULT_ID, 'AMO_CRM', SITE_ID, false, false);

		\Aspro\Functions\CAsproNext::sendResultToIBlock($WEB_FORM_ID, $RESULT_ID);
	}
	...
	public static function OnCommentAddHandler($commentID, &$arFields){
		...
		CNext::updateExtendedReviewsProps($commentID);
	...
	public static function OnCommentDeleteHandler($ID){
		...
		CNext::updateExtendedReviewsProps($ID, 'delete');
	}
}