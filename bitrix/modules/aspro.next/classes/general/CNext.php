...
class CNext{
			...
			<td<?=(!$isArrayItem ? ' width="50%"' : '')?>>
				...
				<?elseif($optionType == "multiselectbox"):?>
					...
					if(isset($arOption['TYPE_SELECT']))
					{
						...
						if ($arOption['TYPE_SELECT'] == 'LINKED_PROP_CATALOG') {
							$iblockIdLink = self::GetFrontParametrValue('CATALOG_IBLOCK_ID', $optionsSiteID);
							
							if ($iblockIdLink) {
								$rsProp = CIBlockProperty::GetList(array("sort"=>"asc", "name"=>"asc"), array("ACTIVE"=>"Y", "IBLOCK_ID"=>($iblockIdLink)));
								
								while ($arr=$rsProp->Fetch()) {					
									if (in_array($arr["PROPERTY_TYPE"], array("L", "N", "S"))) {	
										$arIBlocksLinked[$arr["CODE"]] = "[".$arr["CODE"]."] ".$arr["NAME"];
									}
								}
					...
					<select size="<?=$optionSize?>" <?=$optionController?> <?=$optionDisabled?> multiple name="<?=htmlspecialcharsbx($optionCode)."_".$optionsSiteID?>[]" >
						<? if($bIBlocksLinked): ?>
							<? foreach($arIBlocksLinked as $key => $arValue): ?>
								<? 
									$selected = (!$optionVal && $optionCode === "CATALOG_LINKED_PROP") || ($optionVal && $optionVal === $key) || in_array($key, $optionVal)
										? "selected"
										: "";
								?>
								<option value="<?= $key; ?>" <?= $selected; ?>><?= htmlspecialcharsbx($arValue); ?></option>
							<? endforeach; ?>
						<? else: ?>
							<? for($j = 0, $c = count($arr_keys); $j < $c; ++$j): ?>
	...
	public static function Start($siteID){
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			\Aspro\Next\Functions\Extensions::register();
	...
	public static function getCurrentPageClass(){
		...
		if(!isset($result))
		{
			$result = $APPLICATION->AddBufferContent(array('CNext', 'showPageClass'));
										...
										<?if($arParams['SHOW_DISCOUNT_PERCENT_NUMBER'] != 'Y'):?>
                                            ...
                                            <div class="text">
                                                <span class="values_wrapper"
                                                      ...
                                                      data-value="<?=($diff*$ratio);?>">


                                                    <?=\Aspro\Functions\CAsproItem::getCurrentPrice($diff, $arPrice, false)?>
                                                </span>
                                            </div>
										<?else:?>
											...
											<?}?>
											<div class="text"><?=GetMessage("CATALOG_ECONOMY");?> <span class="values_wrapper">
                                                    <?=\Aspro\Functions\CAsproItem::getCurrentPrice($diff, $arPrice, false);?>
                                                </span>
                                            </div>
										<?endif;?>
	...
	public static function replaceMenuChilds(&$arResult, $arParams) {
		...
		$replaceType = $arTheme['USE_MEGA_MENU']['DEPENDENT_PARAMS']['REPLACE_TYPE']['VALUE'];
	...
	public static function SetJSOptions(){
		...
		var arNextOptions = ({
			...
			"THEME" : ({
				...
				'USE_FAST_VIEW_PAGE_DETAIL': '<?=$arFrontParametrs["USE_FAST_VIEW_PAGE_DETAIL"] !== "NO" ? "Y" : "N"?>',
			}),
	...
	public function checkMask($mask_exc = ''){
		if($mask_exc)
		{
			...
			$arMask = array_diff($arMask, array('', NULL, false));
