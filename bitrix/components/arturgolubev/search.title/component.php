...
$smartcomponent->baseInit(ltrim($_POST["q"]), 'title');
...
)
{
	...
	for($i = 0; $i < $arParams["NUM_CATEGORIES"]; $i++)
	{
		...
		if ($isSearchInstalled)
		{
			...
			if (empty($alreadyFinded) || ($smartcomponent->options['always_use_smart'] && $j < $arParams["TOP_COUNT"])){					
				$time_start2 = microtime(true); 
