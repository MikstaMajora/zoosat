...
foreach($arAvailableSort as $key => $val){
	...
	$url = str_replace('+', '%2B', $current_url);
	$bSelected = ($sort == $val[0] && ($newSortOrder == $sort_order || $key === 'RANK'));
	$arResult['ITEMS']['ASPRO_FILTER_SORT']['VALUES'][] = array(
		'CONTROL_HTML' => '<a href="'.$url.'" class="sort_btn '.($bSelected ? 'current' : '').' '.$newSortOrder.' '.$key.'" rel="nofollow"><span>'.GetMessage('SECT_SORT_'.$key).($key !== 'RANK' ? ' ('.GetMessage($newSortOrder).')' : '').'</span></a>',
		'CHECKED' => $bSelected ? 'Y' : 'N',
		'VALUE' => GetMessage('SECT_SORT_'.$key).($key !== 'RANK' ? ' ('.GetMessage($newSortOrder).')' : ''),
	);
}
?>