...
if(SITE_SERVER_NAME == 'kras.sibagrotrade.com') {
    $arParams['STORES'] = [3, 1];
    ...
    <?php //debug($arResult); ?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<div class="item_block col-<?=$col;?> col-md-<?=ceil(12/$col);?> col-sm-<?=ceil(12/round($col / 2))?> col-xs-6">
				<div class="catalog_item_wrapp item" style = "margin-top: 17px; ">
					...
					<div class="catalog_item main_item_wrapper item_wrap <?=(($_GET['q'])) ? 's' : ''?>" id="<?=$arItemIDs["strMainID"];?>">
						<div>
                            ...
                            <!-- ИКОНКА АКЦИИ НА ТОВАРЫ -->
                            <?php if(!empty($arItem['SALE'])){  ?>
                                <div class = "sale_icon_list" >
                                    <img class = "sale_icon_img" src = "/images/icons/akcia.png">
                                </div>
                            <?php    }  ?>
							...
							<div class="item_info <?=$arParams["TYPE_SKU"]?>">
								...
								<?php if(isset($arItem['PREVIEW_TEXT'])){ ?>
									...
									<div class = "sa_block">
										<div class = "item-stock">
											<span class = "value"><?=my_crop($arItem["PREVIEW_TEXT"], 30); ?></span>
                                ...
                                */ ?>
								...
								<div class="sa_block">
                                    ...
                                    <!---------------------------------------------------------->
                                    <!-- НАЛИЧИЕ НА СКЛАДЕ  23.12.2020 -->
                                    <!---------------------------------------------------------->
                                    <?php if(SITE_ID == 's3'){ ?>

                                        <?php //debug( $arItem['PROPERTIES']['AVAILABLE_NSK'] ); ?>
                                        <?php // debug( $arItem['PROPERTIES']['AVAILABLE_KRAS'] ); ?>
                                        ...
                                        <div class="quantity_block_wrapper"  >
                                            <div class="p_block">
                                                ...
                                                <div class="item-stock" style = "display:block !important; ">
                                                    <span class = "<?php echo ( $arItem['PROPERTIES']['AVAILABLE_NSK']['NEW_VALUE'] != 0 )? 'icon_store_green' : 'icon_store_grey' ?>"> </span>
                                                    <span class = "value">в г. <?=$arItem['PROPERTIES']['AVAILABLE_NSK']['NAME'];?> &nbsp; <span> (<?=$arItem['PROPERTIES']['AVAILABLE_NSK']['NEW_VALUE']?>) </span> </span>
                                                </div>


                                                <div class="item-stock">
                                                    <span class = "<?php echo ( $arItem['PROPERTIES']['AVAILABLE_KRAS']['NEW_VALUE'] != 0)? 'icon_store_green' : 'icon_store_grey' ?>"> </span>
                                                    <span class = "value">в г. <?=$arItem['PROPERTIES']['AVAILABLE_KRAS']['NAME'];?> &nbsp; <span> (<?=$arItem['PROPERTIES']['AVAILABLE_KRAS']['NEW_VALUE']?>) </span> </span>
                                                </div>
                                    ...
                                    <div class="item-stock" style="margin: 0px !important">
                                        <span class="value">Код товара: <b><?php echo $arItem['PROPERTIES']['KOD']['VALUE']; ?> </b>  </span>
                                    </div>
                                ...
                                <?php
                                /// ОТОБРАЖЕНИЕ ЦЕНЫ ДО СКИДКИ
                                /// ЕСЛИ ЦЕНЫ НЕТ ДО СКИДКИ
                                if( empty( $arItem["PRICE_BEFORE_SALE"] )){
                                ?>
								...
								<div class="cost prices clearfix">
									...
									<?}else{?>
										...
										{
                                            if($GLOBALS['USER']->IsAuthorized() && \CSite::InGroup(array(10))){
                                                unset($arItem['PRICE_MATRIX']['COLS'][5]);
                                                unset($arItem['PRICE_MATRIX']['MATRIX'][5]);
                                            }
										    ?>
                                ...
                                <div class="cost prices clearfix">
                                    ...
                                    <?php
                                    $discountPriceForSale = $arItem["PRICE_BEFORE_SALE"] - $arItem['MIN_PRICE']['VALUE'];
                                    ?>
                                    <span class="priceBeforeSaleThroughlineList" >
                                 <?=$arItem["PRICE_BEFORE_SALE"] ?> руб.
                            </span>

                                    <div class="sale_block">
                                        <div class="text">Экономия <span class="values_wrapper"><span class="price_value"><?=$discountPriceForSale;?> руб. </span> </span></div>
                                    </div>
							...
							<div class="footer_button <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?> inner_content js_offers__<?=$arItem['ID'];?>">
                                ...
                                <?php } else {  ?>
                                    ...
                                    <div class="counter_wrapp">
                                        <button class="btn btn-default bold btn-login-sibagro" >Войти</button>
                                    </div>
