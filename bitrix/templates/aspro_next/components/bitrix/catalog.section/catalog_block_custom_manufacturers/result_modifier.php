...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		////////////////////////////////////////////////////////////////////////////
		// ОТОБРАЖЕНИЕ СКЛАДОВ      23.12.2020
		////////////////////////////////////////////////////////////////////////////
		// Новосибирск
		...
		while($arStoreProduct=$rsStoreProduct->fetch()) {
			if($arStoreProduct['AMOUNT'] >= 0){
				$arNewItemsList[$key]['PROPERTIES']['AVAILABLE_NSK']['NEW_VALUE'] = $arStoreProduct['AMOUNT'];
			} else {
				$arNewItemsList[$key]['PROPERTIES']['AVAILABLE_NSK']['NEW_VALUE'] = 0 ;
			}
		...
		while($arStoreProduct=$rsStoreProduct->fetch()) {
			if($arStoreProduct['AMOUNT'] >= 0) {
				$arNewItemsList[$key]['PROPERTIES']['AVAILABLE_KRAS']['NEW_VALUE'] = $arStoreProduct['AMOUNT'];
			} else {
				$arNewItemsList[$key]['PROPERTIES']['AVAILABLE_KRAS']['NEW_VALUE'] = 0;
			}
		}
		////////////////////////////////////////////////////////////////////////////
	...
	// ОТОБРАЖЕНИЕ СКИДКИ ПОД АКЦИИ
	if( CSite::InGroup (array( OPT_GROUP_USERS ) ) ){
		$rsGr = OPT_PRICE_TYPE_BEFORE_SALE;
	} elseif( CSite::InGroup (array( KRUPNOPT_GROUP_USERS ) ) ){
		$rsGr = KRUPN_OPT_PRICE_TYPE_BEFORE_SALE;
	} else {
		$rsGr = OPT_PRICE_TYPE_BEFORE_SALE;
	}
	...
	foreach($arResult['ITEMS'] as $key => $arItem){
		...
		$result = CIBlockElement::GetList(
			...
			$arFilter,
			false, //Группировка
			["nPageSize" => false], //Постраничная навигация. по 2эл-та на страницу
			$arSelect
		);

		while($obj = $result->GetNextElement()){
			$obj->GetFields(); //Получаем из объекта поля
			$arResult['ITEMS'][$key]['PROPERTIES'] = $obj->GetProperties(); //Получаем свойства

		}


		$priceBeforeSale = \Bitrix\Catalog\PriceTable::query()
			->setSelect(['*'])
			->setFilter([
				'=PRODUCT_ID' => $arItem['ID'],
				'=CATALOG_GROUP_ID' => $rsGr,
			])
			->exec()
			->fetch();

		$arResult['ITEMS'][$key]["PRICE_BEFORE_SALE"] = $priceBeforeSale['PRICE'];



		// ОТОБРАЖЕНИЕ ССЫЛКИ "УЧАСТВУЕТ В АКЦИИ"
		// Смотрим в каких разделах находится товар и формируем массив разделов
		$db_old_groups = CIBlockElement::GetElementGroups($arItem['ID'], true);
		$ar_new_groups = Array();
		while($ar_group = $db_old_groups->Fetch()) {
			if( $ar_group['ID'] != $arItem['IBLOCK_SECTION_ID'] ){
		...
		if(!empty($ar_new_groups)){
			...
			$rs = CIBlockElement::GetList(
				false,
				['IBLOCK_ID' => IBLOCK_SALE, 'SECTION_ID' => SECTION_SALE, '!=PROPERTY_COMPLETED_VALUE' => 'YES', 'PROPERTY_LINKED_SECTION_ID' => $ar_new_groups ],
				false, false,
				['ID', 'IBLOCK_ID', 'NAME', 'PROPERTY_*', 'DETAIL_PAGE_URL']
			);

			//debug($rs ->SelectedRowsCount());
			while( $ob = $rs->GetNextElement() ){

				$arResult['ITEMS'][$key]['SALE'][] = $ob->GetFields();
			}
		}
