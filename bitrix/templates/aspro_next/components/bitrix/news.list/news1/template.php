				...
				<div class="col-md-12">
					<div class="item noborder<?=($bImage ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
							...
							<div class="image shine">
								...
								<?endif;?>
									<img src="<?=$imageSrc?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-responsive" />
                                        ...
                                        <!-- АКЦИЯ ЗАВЕРШЕНА -->
										<?php if(!empty($arItem['PROPERTIES']['SALE_COMPLETED']['VALUE'])):  ?>
						...
						<div class="body-info">
							...
							global $USER;
							if ($USER->IsAdmin()){
								//debug($arItem['PROPERTIES']['START_SALE']['VALUE']);
								//debug($arItem['PROPERTIES']['END_SALE']['VALUE']);
							}
							?>

                            <?php if( !empty($arItem['PROPERTIES']['START_SALE']['VALUE']) && !empty($arItem['PROPERTIES']['END_SALE']['VALUE'])): ?>
                                <div class="previewtext">Срок действия акции с  <?=$arItem['PROPERTIES']['START_SALE']['VALUE'];?>  по
                                    <?=$arItem['PROPERTIES']['END_SALE']['VALUE'];?>
                                </div>
                            <?php endif; ?>
