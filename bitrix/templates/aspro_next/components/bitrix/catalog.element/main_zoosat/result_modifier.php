...
<?php
$valueXmlID = $arResult['PROPERTIES']['CML2_MANUFACTURER']['VALUE_XML_ID'];

//Из массива разделов получаем символьный код производителя
function getManufactArr( $valueXmlID ){
	$result = CIBlockSection::GetList(
		array("SORT" => "ASC"),
		array("IBLOCK_ID" => 30, "CODE" => $valueXmlID ),
		false,
...
$arResult['PROPERTIES']['CML2_MANUFACTURER']['ARR_SECTION_MANUFACT'] = getManufactArr( $valueXmlID );
//debug($arSection);



$rsSect = CIBlockElement::GetElementGroups($arResult['ID']);
while ($arSect = $rsSect->Fetch()) {
	//debug( $arSect['ID']);

	if($arSect['ID'] == 558){
		$arResult['CUSTOM_PROPS']['RASPRODAGA'] = $arSect;
	}
}
//debug($arResult);
	...
	foreach($arResult['PROPERTIES']['ADD_MATERIAL_IMG']['VALUE'] as $arItem){
		...
		$arResult['PROPERTIES']['ADD_MATERIAL_IMG']['FINAL_SRC'][$arItem] = $file['src'];
	}
