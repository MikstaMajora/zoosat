...
global $USER;
if ($USER->IsAdmin()){
	//debug($arResult['PROPERTIES']['ADD_MATERIAL_IMG']);
}
...
<div class="item_main_info <?=(!$showCustomOffer ? "noffer" : "");?> <?=($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props");?>" id="<?=$arItemIDs["strMainID"];?>">
	<div class="img_wrapper swipeignore">
		...
		<?php if($arResult['PROPERTIES']['SKIDKA20']['VALUE_XML_ID']){  ?>
            <div class = "sale_icon_card">
                <img src = "/upload/imgs/icons/sale20.png">
            </div>
		<?php    }  ?>

		<?php if($arResult['PROPERTIES']['SKIDKA25']['VALUE_XML_ID']){  ?>
            <div class = "sale_icon_card">
                <img src = "/upload/imgs/icons/zoosat25.png">
            </div>
		<?php    }  ?>

		<?php if($arResult['PROPERTIES']['SKIDKA15']['VALUE_XML_ID']){  ?>
            <div class = "sale_icon_card">
                <img src = "/upload/imgs/icons/zoosat15.png">
            </div>
		<?php    }  ?>

		<?php if($arResult['PROPERTIES']['SKIDKA10']['VALUE_XML_ID']){  ?>
            <div class = "sale_icon_card">
                <img src = "/upload/imgs/icons/zoosat10.png">
            </div>
		<?php    }  ?>
		...
		<?php if(!empty($arResult['CUSTOM_PROPS']['RASPRODAGA'])): ?>
        ...
        <div class = "stickers">
            <!-- Иконка для заводчиков -->
                ...
                <?php if( $arResult["PROPERTIES"]["ZAVODCHIKAM"]["VALUE"] == "Да"): ?>
                    <?php $img_root = '/upload/imgs/zavod_sticker.png'; ?>
                    <a href = "/help/zavodchikam/" target = "_blank">
                        <img src = "<?=$img_root;?>" title = "Товар для заводчиков">
                    </a>
                <?php endif; ?>
        </div>
	...
	<div class="right_info">
		<div class="info_item">
            ...
            <div class="code_block">
                <?php  //////////////////////// Код товара /////////////////////////////// ?>
                <?php if( $arResult["PROPERTIES"]["KOD"]["VALUE"] ): ?>
                    Код товара:&nbsp <?=$arResult["PROPERTIES"]["KOD"]["VALUE"];?>
                <?php endif; ?>

                <?php if( $arResult["PROPERTIES"]["KOD"]["VALUE"] ): ?>
                    &nbsp &nbsp Артикул:&nbsp <?=$arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"];?>
                <?php endif; ?>
            </div>
            ...
            <?php /*
            <div class = "zavodchikam_block_element">
                <?php if( $arResult["PROPERTIES"]["ZAVODCHIKAM"]["VALUE"] == "Да"): ?>
                    <?php $img_root = '/upload/imgs/zavod.jpg'; ?>
                    <a href = "/help/zavodchikam/" target = "_blank">
                        <img src = "<?=$img_root;?>">
                    </a>
                <?php endif; ?>
            ...
            */ ?>
            ...
            <?php if($arResult['PROPERTIES']['CML2_MANUFACTURER']['VALUE']): ?>
            <div>

                Производитель: <a href = "/manufacturers/?SECTION_ID=<?=$arResult['PROPERTIES']['CML2_MANUFACTURER']['ARR_SECTION_MANUFACT']['ID'];?>" target = "_blank"><?=$arResult['PROPERTIES']['CML2_MANUFACTURER']['ARR_SECTION_MANUFACT']['NAME'];?></a>

                <?php
                // Вывод для админа
                global $USER;
                if ($USER->IsAdmin()){
                    //debug($arResult['PROPERTIES']['CML2_MANUFACTURER']['VALUE']);
                }
            ...
            <?php if( !empty($arResult['PROPERTIES']['SHTRIKHKOD']['VALUE'])): ?>
            <div class="sibagro_barcode">
                ...
                <div>
                    ...
                    $barCode = $arResult['PROPERTIES']['SHTRIKHKOD']['VALUE'];
                    echo '<img src="data:image/png;base64,' . base64_encode($generator->getBarcode("$barCode", $generator::TYPE_CODE_128 )) . '">';
                    ?>
                </div>
                <div class="sibagro_barcode_value"><?=$arResult['PROPERTIES']['SHTRIKHKOD']['VALUE'];?></div>
            </div>
            <?php endif; ?>

            <?php if($arResult['PROPERTIES']['ADD_MATERIAL_IMG']['FINAL_SRC']): ?>
            <div>
                ...
                <?php foreach($arResult['PROPERTIES']['ADD_MATERIAL_IMG']['FINAL_SRC'] as $item):  ?>
                    <div class="column">
                        <link href="<?=$item;?>" itemprop="image">
...
if (!empty($arResult[PROPERTIES][FILES][VALUE]) ){
    ...
    foreach($arResult[PROPERTIES][FILES][VALUE] as $arItem):
        ...
        $resArrItems = CFile::GetByID( $arItem );

        $arrItems = $resArrItems->GetNext();

        //debug( $arrItems );


        $extensionFile = substr($arrItems['FILE_NAME'], -4, 4);

        if( $extensionFile == '.pdf' ){

            $resultPdf[] = $arrItems;

        } elseif( $extensionFile == '.htm'){

            $firstElementInArrItems = substr( $arrItems['DESCRIPTION'], 0, 1);

            $resultHtml[$firstElementInArrItems] = $arrItems;
    ...
    <div class = "tabs">
        ...
        <ul class = "nav nav-tabs">
            <?php foreach($resultHtml as $key => $item): ?>

                <?php $item['DESCRIPTION'] = substr($item['DESCRIPTION'],1); ?>

                <?php if($key == 1){ ?>
                    <li class = "active"><a href = "#tab<?=$key;?>" data-toggle = "tab"> <?=$item['DESCRIPTION'];  ?> </a></li>
                <?php } else { ?>

                    <li><a href = "#tab<?=$key; ?>" data-toggle = "tab"  > <?=$item['DESCRIPTION'];  ?> </a></li>
            ...
            <?php if(!empty( $resultPdf )){ ?>

                <li><a href = "#tab10" data-toggle = "tab"> Дополнительные материалы </a></li>
        ...
        <div class = "tab-content">
            ...
            <?php foreach($resultHtml as $key => $item): ?>
                ...
                <?php  $pathFile = CFile::GetPath( $item['ID'] );  ?>



                <?php if($key == 1){ ?>

                    <div class="tab-pane   in active" id = "tab<?=$key; ?>" >


                        <?php

                        $endPath = $_SERVER['DOCUMENT_ROOT']."$pathFile";

                        $file = file_get_contents("$endPath" );
